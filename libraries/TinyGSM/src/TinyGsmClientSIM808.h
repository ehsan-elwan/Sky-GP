...
public:
  ...
  // works only with ans SIM808 V2
  bool getGPS(double *lat, double *lon, double *speed=0, int *alt=0, int *vsat=0, int *usat=0) {
    //String buffer = "";
    char chr_buffer[12];
    bool fix = false;
    ...
    *lat =  stream.readStringUntil(',').toDouble(); //lat
    *lon =  stream.readStringUntil(',').toDouble(); //lon
    if (alt != NULL) *alt =  stream.readStringUntil(',').toDouble(); //lon
    if (speed != NULL) *speed = stream.readStringUntil(',').toDouble(); //speed
